...
- 问题数量保持克制，回复长度按风险阶段、用户状态和对话意图动态调整。
...
`crisis_response`, `boundary_response`, `clinical_red_flag_response` 不再写整段固定模板，而是调用 `risk_response_policy` 生成动态长度回复。第一版可以仍用受控模板片段组合，后续再切到模型生成。
...
回复不应始终很短。短回复适合即刻风险首轮，因为用户可能处在行动边缘，长段解释会稀释“先停住”的目标。但当用户继续回应、表达愿意说、或明确需要陪伴时，系统应该适度展开，让用户感到有人真的在听，而不是只收到安全指令。

建议的长度策略：

| 场景 | length_profile | 目标长度 | 说明 |
| --- | --- | --- | --- |
| `L3` 首轮、`active` 或 `near_term` | `brief_first_contact` | 120-260 字 | 先稳定和确认回应，避免长篇说理 |
| `L3` 持续高风险但用户仍回应 | `steady_short` | 180-360 字 | 维持陪伴感，每轮只推动一个小动作 |
| `L2` 或 `L3` 开始缓和 | `warm_medium` | 260-520 字 | 可以更多共情、复述处境、提供一个温和练习 |
| `L1` 情绪困扰 | `supportive_medium` | 320-700 字 | 允许较完整的陪伴、澄清和轻量建议 |
| 用户明确说“多陪我说说/讲点什么” | `holding_longer` | 500-900 字 | 可用短比喻或温柔叙述，但仍避免空泛鸡汤 |
| 边界/用药/诊断/prompt attack | `firm_brief` | 120-300 字 | 清楚守界，快速回到安全替代方向 |

动态长度规则：

- `max_questions` 通常为 1；当回复是纯陪伴、没有探索问题时可以为 0。
- `L3` 首轮不追求长共情，优先确认用户还在、让动作延后。
- 用户连续回应后，可以加长到中等长度，包含更多情绪理解和身体稳定提示。
- 用户抗拒“安全流程”时，不增加解释长度，而是降低指令感。
- 长回复不能变成长故事、生命意义说教或“世界很美好”式劝活。
- validator 不按固定字数一刀切，而按 `length_profile` 检查是否超出对应预算。
...
- 回复长度明显超过当前 `length_profile` 的预算，尤其是即刻风险首轮长篇说理。
- 一轮多个问题。
- 复述具体危险工具/地点/动作。
...
11. 语音入口复用同一后端策略字段。
12. 现有安全分类测试继续通过。
13. 不同 `length_profile` 的回复长度符合动态预算：`L3` 首轮短，缓和后可中等展开，用户请求陪伴时允许更长。
