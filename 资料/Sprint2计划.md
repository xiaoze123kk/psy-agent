...
- 占位：knowledge、tests、voice 路由已有 scaffold，但还不是可用产品功能。
...
- 语音会话 WebSocket 从 echo scaffold 升级为“文本模拟话轮”技术验证。
- 用户反馈接口：对知识答案、测试结果、AI 回复做 1-5 分反馈。
- 每周情绪小结生成。

### 4.3 本 Sprint 不做

- 完整实时语音 ASR / TTS / WebRTC。
- 动漫角色完整角色库和分享卡片。
...
- mood 趋势按 7d / 30d 聚合。
- 用户设置接口，用于更新 memory_mode、companion_style、voice_enabled。
- 安全评测脚本和样例数据。

### 5.3 Agent 交付

- knowledge_graph 或 knowledge 节点第一版。
...
4. 用户进入知识页，搜索“焦虑依恋”，打开知识详情。
5. 知识详情提供 30 秒解释、3 分钟解释、行动建议和现实求助边界。
6. 用户点击“继续聊聊这个问题”，进入对话页，AI 能带着知识上下文回应。
7. 用户进入测试页，完成今日状态测试。
...
| E10 语音 Spike | WebSocket 文本模拟话轮、状态事件 | P1 | 不接真实音频也能验证协议 |
...
- 风险：Sprint 2 同时做知识、测试、记忆和情绪，范围偏大。
  - 缓解：P0 只做最小可用闭环；动漫和完整语音放 P1 / Sprint 3。
- 风险：测试结果被用户理解成诊断。
