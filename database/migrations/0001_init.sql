...
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    user_mode VARCHAR(16) NOT NULL CHECK (
        user_mode IN ('teen', 'adult')
    ),
...
CREATE TABLE IF NOT EXISTS user_settings (
    user_id UUID PRIMARY KEY REFERENCES users (id) ON DELETE CASCADE,
    memory_mode VARCHAR(24) NOT NULL DEFAULT 'summary_only' CHECK (
        memory_mode IN (
            'off',
            'summary_only',
            'long_term'
...
CREATE TABLE IF NOT EXISTS conversation_threads (
    ...
    last_summary TEXT,
    last_risk_level VARCHAR(8) NOT NULL DEFAULT 'L0' CHECK (
        last_risk_level IN ('L0', 'L1', 'L2', 'L3')
    ),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS user_memories (
    ...
    visibility VARCHAR(24) NOT NULL DEFAULT 'user_visible' CHECK (
        visibility IN (
            'user_visible',
            'internal_safety'
        )
...
CREATE TABLE IF NOT EXISTS tests (
    ...
    test_type VARCHAR(24) NOT NULL CHECK (
        test_type IN (
            'state',
            'personality',
            'anime'
...
CREATE TABLE IF NOT EXISTS test_attempts (
    ...
    status VARCHAR(24) NOT NULL DEFAULT 'in_progress' CHECK (
...
CREATE INDEX IF NOT EXISTS idx_threads_user_updated_at ON conversation_threads (user_id, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_memories_user_status_updated ON user_memories (
    user_id,
    status,
    updated_at DESC
);

CREATE INDEX IF NOT EXISTS idx_mood_logs_user_created_at ON mood_logs (user_id, created_at DESC);
